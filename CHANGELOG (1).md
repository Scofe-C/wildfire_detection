...
### Added
- **`src/components/landing/LandingPage.jsx`** — Product landing page
  - Hero section: "AI Automatic Reporting Fire Detection" with "Detect · Predict · Respond" subtitle
  - Two feature cards: Real-Time Monitoring and AI-Generated Reports
  - "Enter Dashboard" CTA leads to Fire Detection Map as default view
  - Clicking Wildfire logo in sidebar returns to landing page
...
- **`src/components/layout/Sidebar.jsx`** — Logo click navigates back to landing page
...
- **`src/components/fire-map/FireMap.jsx`** — Mobile responsive & click fix
  ...
  - Cell Detail panel: Close button now always visible (not just when a cell is selected)
  - Cell Detail panel: closing hides the entire panel; a "Cell Detail" button appears on the map to reopen
  - Cell Detail panel: clicking a cell auto-reopens the panel if it was closed
  - Cell Detail panel: auto-selects the highest risk fire cell on first load (prioritizes active fires)
...
### Sharing on Local Network (Mobile / Tablet / Other Devices)

To let others on the same WiFi access the dashboard:

```bash
npm run dev -- --host
```

Terminal will show a Network URL like `http://192.168.x.x:5173/`. Use that IP.
...
python3 -c "
import qrcode
url = 'http://<YOUR_IP>:5173/?mode=viewer'
img = qrcode.make(url)
img.save('qrcode.png')
...
Replace `<YOUR_IP>` with your actual Network IP from the terminal output.

### Access Modes

| Mode | URL | Description |
|------|-----|-------------|
| **Full (Admin)** | `http://localhost:5173/` | Landing page → full dashboard with all features, sidebar, editing |
| **Viewer (Read-only)** | `http://<IP>:5173/?mode=viewer` | Fire Detection Map + Incident Reports only, no sidebar, no edit |

### Navigation

- **Landing Page** → Click "Enter Dashboard" → enters Fire Detection Map
- **Sidebar Logo** → Click the Wildfire flame icon to return to landing page
- **Viewer Mode** → Top bar tab switcher between Fire Detection Map and Incident Reports

### Key Files Modified

| File | What it does |
|------|-------------|
| `src/App.jsx` | Routing: landing page, viewer mode, default view (fire-map) |
| `src/components/landing/LandingPage.jsx` | Product landing page |
| `src/components/layout/Sidebar.jsx` | Logo click → back to landing |
| `src/components/layout/Header.jsx` | Dynamic system date/time |
| `src/components/fire-map/FireMap.jsx` | Mobile responsive, close button, click-through fix, LayerBar fix |
| `src/components/reports/IncidentReports.jsx` | Mobile responsive layout |
