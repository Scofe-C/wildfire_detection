...
def run_obj2(df: pd.DataFrame, top_n: int = 1) -> None:
    ...
    from models.obj2_spread.spread_metrics import analyze_threatened_cells, compute_input_quality
    ...
    for _, cell_row in top_cells.iterrows():
        ...
        try:
            ...
            print(f"  direction   : {h_dir:.1f} deg  (dominant: {result['dominant_factor']})")
            print(f"  speed       : {h_speed:.4f} km/h  (det={det_speed:.4f}, p90={mc_p90:.4f})")
            print(f"  moisture    : {result['dead_fuel_moisture_pct']:.1f}%  (RH={inputs.get('relative_humidity_pct','?')}%)")
            ...
            print(f"  slope       : {inputs.get('ignition_cell_slope_deg', 0):.1f} deg  aspect {inputs.get('ignition_cell_aspect_deg', 0):.0f} deg")
            print(f"  fuel model  : FBFM {int(inputs['ignition_cell_fbfm40']) if inputs.get('ignition_cell_fbfm40') else 'N/A'}")
            print(f"  crown prob  : {mc.get('crown_fire_probability', 0):.1%}")

            print(f"\n  Distance projection (hybrid {h_speed:.4f} km/h):")
            for hr in [1, 2, 3, 6]:
                dist = h_speed * hr
                print(f"    t={hr}h : {dist:.3f} km  ({dist/25*100:.1f}% of 25km intercell)")
